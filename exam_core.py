...
# ========== 2. 【新增】清理简答题答案里的英文字母序号 ==========
def clean_short_answer_prefix(answer_text: str) -> str:
    """
    清理简答题答案里的A. B. C. D. 等英文字母序号
    自动把英文字母序号替换成1.2.3.数字序号，保证格式规范
    """
    if not answer_text:
        return answer_text
...
def grade_exam_answers(
    ...
    for i in range(total_question_num):
        ...
        # 【修改】简答题批改前，先清理正确答案里的A.B.C.D.
        if current_q_type == "简答":
            correct_ans = clean_short_answer_prefix(correct_ans)
...
def cosine_similarity(vec1: List[float], vec2: List[float]) -> float:
    """计算两个文本向量的余弦相似度（替代sentence_transformers工具函数）"""
...
def get_short_answer_score(
    ...
    for idx, point in enumerate(score_points):
        ...
        if model is not None:
            try:
                # 调用阿里云API获取向量（替代 model.encode()）
                user_embedding = model(user_ans_clean)
                point_embedding = model(point_clean)
                # 计算余弦相似度
                similarity = cosine_similarity(user_embedding, point_embedding) * 100
                is_hit = similarity >= SIMILARITY_THRESHOLD
                mode_used = "语义匹配(阿里云API)"
            except Exception as e:
                print(f"阿里云API调用失败，降级关键词匹配：{str(e)}")
...
def grade_exam_answers(
    ...
    :param current_q_type: 题型
    :param total_question_num: 总题数
    :param model: 阿里云向量API函数，用于简答题评分
    :return: 总分, 得分详情列表
    """
    total_score = 0
    score_detail = []
    ...
    for i in range(total_question_num):
        ...
        if current_q_type == "单选":
            ...
            if not model:
                ...
                is_correct = get_score >= question_total_score * 0.99
                status = "full" if is_correct else "partial" if get_score > 0 else "wrong"
